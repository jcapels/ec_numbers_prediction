...
from pipeline.filter_uniref import FilterByUniRef90
...
class EnrichmentWithTrembl(luigi.Task):
    ...
    def get_sequences_per_ec(self, swiss_prot_dataframe, take_out_incomplete_ecs=True, level=4):
        ...
        if level == 1:
            ...
            df["EC"] = df["EC"].apply(lambda x: [re.search(r"^\d+.\d+", i).group() for i in x if re.search(r"^\d+.\d+", i)])
        elif level == 3:
            df["EC"] = df["EC"].apply(lambda x: [re.search(r"^\d+.\d+.\d+", i).group() for i in x if re.search(r"^\d+.\d+.\d+", i)])
        elif level == 4:
            df["EC"] = df["EC"].apply(lambda x: [re.search(r"^\d+.\d+.\d+.n*\d+", i).group() for i in x if re.search(r"^\d+.\d+.\d+.n*\d+", i)])
    ...
    def create_enriched_dataset(self, sequences_per_ec_less_than_100):
        ...
        ecs_for_enrichment_dict = dict(zip(sequences_per_ec_less_than_100.loc[:,"EC"], sequences_per_ec_less_than_100.loc[:,"num_sequences"]))
        ...
        for _, row in trembl.iterrows():
            ...
            if len(ECs) < 2:
                for EC in ECs:
                    # level 4 enrichment
                    if EC in ecs_for_enrichment_dict_keys and ecs_for_enrichment_dict[EC] < 100 and row["accession"] not in sequences_added:
