...
def transform_datasets(transformers: List[Transformer], save_folder: str, dataset_csv: str,
                       batch_size: int = None):
    """
    ...
    dataset = SingleInputDataset.from_csv(dataset_csv, instances_ids_field="accession", representation_field="sequence",
                                          labels_field=3, batch_size=batch_size)

    for transformer in transformers:
        transformer.fit(dataset)
        transformer.transform(dataset)

    dataset.save_features(f"{save_folder}")
...
def generate_one_hot_encodings(dataset_directory: str, save_folder: str):
    ...
    transform_datasets(transformers, save_folder, dataset_csv=dataset_directory)
